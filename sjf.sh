...
echo "SJF Scheduling (Non-Preemptive and Preemptive)"
...
WaitingTime_TurnAroundTime waiting_time completion_time arrival_time burst_time_reserve tat
insertion_sort_pid arrival_time burst_time_reserve pid completion_time waiting_time tat
print_results pid arrival_time burst_time_reserve waiting_time tat completion_time
compute_averages waiting_time tat AvgWT AvgTat

echo "Average waiting time: $AvgWT"
echo "Average turnaround time: $AvgTat"

# Average waiting time and turnaround time using Gantt Chart
...
for ((i = 0; i < n; i++)); do
    read -p "Enter the arrival time for process P$((i + 1)): " at[i]
    read -p "Enter the burst time for process P$((i + 1)): " burst_time[i]
...
echo -e "Average turnaround time = $(awk "BEGIN {printf \"%.2f\", $sumt/$n}")"
