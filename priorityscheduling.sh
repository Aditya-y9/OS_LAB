...
read -p "Enter number of processes: " numP

# Arrays to store arrival time, burst time, priority, completion time, turnaround time, and waiting time
declare -a arrivalT burstT prio isCompleted completionTime taT waitingT

# Initialize arrays
for ((i = 0; i < numP; i++)); do
    read -p "Arrival Time of p$(expr $i + 1): " arrivalT[$i]
    isCompleted[$i]=0
done
...
echo "--- Higher number indicates greater priority ---"
for ((i = 0; i < numP; i++)); do
...
while [ $nProcessDone -lt $numP ]; do
    ...
    for ((i = 0; i < numP; i++)); do
        if [[ (isCompleted[i] -eq 0) && (arrivalT[i] -le timeC) && (maxPrioIndex -eq -1) ]]; then
            ((maxPrioIndex = i))
        fi
...
echo "Number of Context Switches: $contextSwitches"
