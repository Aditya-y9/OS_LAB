...
// Function to simulate Random page replacement algorithm
void* random_page_replacement(void* args) {
    int length = *((int*) args);
    int capacity;
    ...
    for (int i = 0; i < length; i++) {
        ...
        if (!found) {
            if (top < capacity) {
                ...
                int replace_index = rand() % capacity; // Randomly select a page to replace
...
int main() {
    ...
    pthread_create(&thread, NULL, random_page_replacement, &length);
